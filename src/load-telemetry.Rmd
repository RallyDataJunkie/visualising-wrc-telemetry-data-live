...
```{webr setup_load_telem, warning=FALSE, include=FALSE, echo=FALSE, message=FALSE, output=FALSE}
library(knitr)
library(tidyr)
library(purrr)
library(dplyr)
...
```{webr setup_load_telem_utils, warning=FALSE, include=FALSE, echo=FALSE, message=FALSE, output=FALSE}
...
route_telem_intersect = st_intersects(buffered_route_utm, route_telem_utm)

# And then filter on those points
# Also nullify the Z dimension
route_telem = route_telem_utm[route_telem_intersect[[1]],]  %>% st_zm()
```
...
# But we have managed to plot leaflet routes previously...
leaflet(route_telem )  %>% 
  addProviderTiles("OpenTopoMap", group = "OSM") %>% 
  addCircleMarkers()
```
...
dist_points <- st_line_sample(route_sf, 
  ...
  st_nearest_points(points_sf) %>% # Find nearest points between line points and input points
  st_length()


```{webr find_points_on_route}
# NOT FOUND? TO DO
library(rgeos)
...
# st_line_project in sf for a single point?
# https://github.com/r-spatial/sf/issues/2291
# or st_nearest_points ?
...
TO DO
# Assuming stage_route_utm is your route linestring
# Convert to sf if not already
route_sf <- st_as_sf(stage_route_utm)

# Create point at specific distance along the line
sample_point_sf_utm <- st_line_sample(route_sf, 
                                     distance = 5000) %>% # Distance in meters
                      st_cast("POINT") # Convert from MULTIPOINT to POINT if needed

# Transform to latlong CRS
sample_point_sf <- sample_point_sf_utm %>% 
                   st_transform(crs = st_crs(latlon_crs))

BUT distance=5000 is wrong; instead use density, but set how?


instead

# Get point at 5000m along the line
sample_point_sf_utm <- st_line_interpolate_point(route_sf, dist = 5000/st_length(route_sf))
...
"""
```{webr find_point_5km_in, warning=FALSE}
