...
telem_df_full_raw = read.csv(file.path(path, "df_telemetrymergeddata_SS7_Evans.csv") )  %>% drop_na("lon") %>% 
                  map_df(rev) %>%
  mutate(delta_s=cumsum(c(0,diff(utx)))/1000)%>%
                  mutate(utc = as_datetime(utx/1000)) %>%
                  mutate(delta_s = replace(delta_s, which(delta_s<0), 0) ,
                         t=cumsum(delta_s))

telem_df_full = telem_df_full_raw  %>%
                  st_as_sf(coords = c("lon","lat")) %>% st_set_crs(4326)

# Also generate a utm projection
...
The  full telemetry is more localised anyway:
...
utm_telem_full = st_transform(telem_df_full, crs=utm_proj4_string) %>% 
